...
**Tecleo en el editor (capa de UX, no cambia la gramática/almacenamiento anterior):** el
campo de texto añade el espacio separador automáticamente al completar cada token, así que
ya no hace falta pulsarlo a mano entre notas. Los bemoles se escriben con `.` delante de la
letra (`.C` → `Cb`) en vez de pegando una `b` detrás, porque una `b` suelta tiene que seguir
significando siempre la nota Si — ver D12.
...
| D10 | `.staff-scroll` NO lleva `scroll-behavior: smooth` en CSS | El seguimiento horizontal dentro de la línea activa (Tarea 7) asigna `scrollLeft` en cada frame de reproducción sobre el MISMO elemento (`.staff-scroll`, contenedor 2-ejes compartido); con `scroll-behavior: smooth` puesto en CSS, cada asignación arrancaba una animación suave que el siguiente frame interrumpía — persecución perpetua sin converger. Encontrado por el revisor de la Tarea 5 | revisada (el cierre sobre que el salto vertical no perdía suavidad era incorrecto, ver D11) | 2026-09-07 |
| D11 | El salto vertical (`rowEl.scrollIntoView(...)`) también pierde `behavior:"smooth"` — pasa a ser instantáneo, igual que el seguimiento horizontal | D10 asumía que el `behavior:"smooth"` explícito de `scrollIntoView` sobrevivía a quitar la propiedad CSS, por ser una opción a nivel de llamada. Cierto para la suavidad, falso para que la animación LLEGUE a completarse: en una misma caja de scroll, cualquier escritura de scroll (suave o instantánea, en cualquier eje) cancela una animación de scroll suave ya en marcha en esa misma caja — comportamiento estándar de CSSOM View, no un artefacto del navegador headless. Como el seguimiento horizontal escribe `scrollLeft` en cada frame durante toda la reproducción, cancelaba la animación vertical antes de que se moviera ni un píxel, cada vez que cruzaba de línea. Encontrado por el implementador de la Tarea 7 con reproducción de audio real (1107 frames muestreados, `scrollTop` nunca salió de 0 en un cruce) y confirmado con una repro sintética aislada (una escritura de `scrollLeft` por frame, sin nada más, basta para bloquear un `scrollIntoView({behavior:"smooth"})` en marcha en el mismo elemento). El salto de línea de la tira de texto (elemento distinto, sin escritura competidora por frame) conserva su suavidad vía la CSS de la Tarea 4, sin cambios | vigente | 2026-09-08 |
| D12 | El editor añade el espacio separador solo al completar cada token (letra/silencio/compás) tecleado en un límite de token, vía `onKeyDown` en `Editor.jsx` reenviando a `onText` (reutiliza el pipeline de D7-D9 sin tocarlo); los bemoles se escriben con `.` delante de la letra (`.C` → `Cb`), no con `b` pegada detrás | Objetivo del usuario: no tener que pulsar espacio a mano entre notas. Se descartó fundir una `b` suelta como bemol automático porque `b` es ambigua (letra Si vs. alteración) y forzaba "pensar si esta b se puede confundir" antes de cada pulsación — el usuario prefirió un gesto aparte y explícito. Primer candidato, `´` (acento) delante de la letra: descartado, es tecla muerta en teclados ES/Latino — la composición a nivel de SO competía con la reescritura del `value` controlado de React y corrompía texto y selección (confirmado por el usuario: `´b`→`bbb`, `´c`→`cbc`, y el campo completo quedaba seleccionado tras la corrupción). Segundo candidato, `,` (coma): descartado sin probar, ya es el sufijo de octava grave — reabría la misma ambigüedad de posición que motivó cambiar de `´`. `.` no es tecla muerta en ningún teclado, no tiene uso previo en la gramática y no requiere Mayús en teclado ES | vigente | 2026-09-23 |
