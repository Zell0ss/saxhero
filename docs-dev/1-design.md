...
| D10 | `.staff-scroll` NO lleva `scroll-behavior: smooth` en CSS; el salto vertical de línea sigue siendo suave porque `scrollIntoView({behavior:"smooth"})` lo controla explícitamente por llamada, independiente de la propiedad CSS del elemento | El seguimiento horizontal dentro de la línea activa (Tarea 7) asigna `scrollLeft` en cada frame de reproducción sobre el MISMO elemento (`.staff-scroll`, contenedor 2-ejes compartido desde D-pentagrama-multilínea); con `scroll-behavior: smooth` puesto en CSS, cada asignación arrancaba una animación suave que el siguiente frame interrumpía — persecución perpetua sin converger. Encontrado por el revisor de la Tarea 5 al confirmar que el trabajo asignado a la Tarea 7 sería cubrible; el salto vertical no pierde suavidad porque su `behavior:"smooth"` explícito por llamada no depende de la propiedad CSS | vigente | 2026-09-07 |
